...
<head>
  ...
  <style>
    :root { --brand:#005dab; --bg:#fafafa; --ink:#111; }
    *{box-sizing:border-box}
    ...
    h1{margin:2px 0 10px 0;font-size:20px}
    ...
    @media print{
      header{display:none}
      .series{page-break-before:always;border:0;box-shadow:none}
      body{background:#fff}
    }
  </style>
</head>
...
function fmtDates(arr){
  return arr.map(s=>`${s.slice(8,10)}.${s.slice(5,7)} kl. ${s.slice(11,16)}`).join(', ');
}
function cardHTML(it){
  return `<div class="card">
    ...
    <div>
      ...
      ${it.synopsis ? `<div>${it.synopsis.replace(/\n{2,}/g,'<br><br>')}</div>` : `<div class="muted">Ingen synopsis</div>`}
...
$('run').addEventListener('click', async () => {
  ...
  try{
    ...
    $('out').innerHTML = '<p>Ingen visninger i valgt interval.</p>';
  }finally{
    $('run').disabled = false;
  }
});
